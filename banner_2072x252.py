...
FONT_URLS = {
    ...
    "BebasNeue-Bold":         f"{_GF}/ofl/bebasnue/BebasNeue-Regular.ttf",
    "Anton-Regular":          f"{_GF}/apache/anton/Anton-Regular.ttf",
    "Anton-Bold":             f"{_GF}/apache/anton/Anton-Regular.ttf",
...
def _build_layout_prompt(components: list[dict], target_w: int, target_h: int) -> str:
    ...
    return f"""You are a professional ad designer. Arrange these ad components on a horizontal billboard banner.

TARGET CANVAS: {target_w} × {target_h} pixels (ultra-wide, thin)

COMPONENTS:
{chr(10).join(lines)}

LAYOUT RULES:
1. EVERY component must appear — none omitted
2. id=0 (background) must be: x=0, y=0, width={target_w}, height={target_h}
3. Arrange left-to-right: logo/brand → headline → body text → CTA → fine print
4. ALL elements must fit within y: 0–{target_h} (hard limit)
5. Primary text max height ~120px, secondary ~60px, fine print ~30px
6. QR codes minimum 80×80px
7. 20px horizontal padding between elements
8. Center elements vertically: y = ({target_h} - element_height) / 2

Return ONLY compact JSON on a single line (no markdown, no explanation, no whitespace):
{{"layout":[{{"id":0,"x":0,"y":0,"width":{target_w},"height":{target_h}}},{{"id":1,"x":20,"y":0,"width":300,"height":{target_h}}},...]}}"""
...
YOUR ONLY TASK:
Arrange these EXACT components into a single {target_w}×{target_h} ultra-wide horizontal banner (8:1 aspect ratio). This is a LAYOUT job, not a drawing job: DONOT DUPLICATE ANY OF THE COMPONENETS OR MODIFY ITS FEATURES AND DONOT DEUPLICATE ANY BACKGROUND KEEP IT CONSISTENT.

ABSOLUTE RULES — violating any of these is a critical failure:

1. COPY-PASTE FIDELITY — ZERO TOLERANCE FOR REDRAWING:
   - Every component MUST be a pixel-accurate copy from the reference images.
   - Faces, people, hands, products: IDENTICAL to the reference. Same expression, same skin tone, same clothing. If you cannot copy it perfectly, show the component exactly as-is from the reference grid.
   - Logos and brand marks: IDENTICAL. Do not simplify, redraw, or approximate.
   - QR codes: IDENTICAL pixel-for-pixel. Any change makes them unscannable.
   - Text: IDENTICAL characters, font weight, and color.

2. NO DUPLICATION — STRICT ONE-INSTANCE RULE:
   - Each component appears EXACTLY ONCE in the final banner.
   - NEVER repeat a person, product, hand, or any foreground element to fill space.
   - Empty horizontal space = extend the background only (solid color, gradient, or texture that matches the brand palette).

3. DIMENSIONS:
   - Output is ONE continuous image, extremely wide and thin ({target_w}×{target_h}, 8:1 ratio).
   - Do NOT output multiple versions, stacked layouts, or vertical images.
   - Fill edge-to-edge with no borders or letterboxing.

4. SCALING:
   - Scale components DOWN proportionally to fit the {target_h}px banner height.
   - NEVER stretch, squash, or distort any element.

5. TEXT AND TYPOGRAPHY PRESERVATION — CRITICAL:
...
Output ONLY the final {target_w}×{target_h} banner. Nothing else."""
...
async def main():
    ...
    parser.add_argument("--width",  type=int, default=2070)
    parser.add_argument("--height", type=int, default=253)
