...
const colors = {
  ...
  // Selection variants for interactive badges
  "primary-selected": "bg-primary text-white",
  "primary-unselected": "bg-gray-100 text-gray-700 hover:bg-gray-100",
};

type BadgeProps = { color?: keyof typeof colors };
type BadgeExtraProps = { fullWidth?: boolean };
...
export function Badge({
  ...
  fullWidth,
  ...props
}: BadgeProps & BadgeExtraProps & React.ComponentPropsWithoutRef<"span">) {
  return (
    <span
      {...props}
      className={clsx(
        className,
        "flex items-center justify-center gap-x-1.5 rounded-badge px-badge-px py-badge-py h-badge text-sm font-medium forced-colors:outline whitespace-nowrap px-4",
        // badge width/responsiveness is controlled by the container (badge-group)
        // keep a small min width for pill shape
        "min-w-[80px]",
        colors[color]
      )}
    >
      {(props as React.PropsWithChildren).children}
    </span>
  );
}
...
export const BadgeButton = forwardRef(function BadgeButton(
  {
    ...
    BadgeExtraProps & { className?: string; children: React.ReactNode } & (
  ...
  const classes = clsx(
    className,
    "badge-button group relative rounded-md focus:not-data-focus:outline-hidden data-focus:outline-2 data-focus:outline-offset-2 data-focus:outline-blue-500 inline-flex cursor-pointer"
  );
