...
function StatusChip({ status }: { status?: ComplianceStatus | null }) {
    ...
    return (
        <Chip
            label={status}
            size="small"
            sx={{ bgcolor: c.bg, color: c.color, fontWeight: 600, fontSize: "11px", height: 22 }}
        />
    );
}
...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
    return (
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", py: 0.6 }}>
            ...
            <Typography sx={{ fontSize: "12.5px", color: "#111827", fontWeight: 500, textAlign: "right" }}>
                {value}
            </Typography>
        </Box>
    );
...
function Section({ title, status, children }: { title: string; status?: ComplianceStatus | null; children: React.ReactNode }) {
    return (
        <Paper elevation={0} sx={{ border: "1px solid #E5E7EB", borderRadius: "18px", p: 2, flex: 1, minWidth: 220 }}>
            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1 }}>
                <Typography sx={{ fontSize: "13px", fontWeight: 600, color: "#111827" }}>{title}</Typography>
...
export default function ComplianceReportModal({
    ...
    return (
        <Modal open={open} onClose={onClose} sx={{ display: "flex", alignItems: "center", justifyContent: "center", p: 2 }}>
            <Box
                id="compliance-report-printable"
                sx={{
                    position: "relative",
                    width: "100%",
                    maxWidth: 760,
            ...
            >
                <style>{`
                    @media print {
                        ...
                        #compliance-report-printable, #compliance-report-printable * { visibility: visible; }
                        #compliance-report-printable { position: absolute; left: 0; top: 0; width: 100%; max-height: none; overflow: visible; box-shadow: none; }
                        #compliance-report-print-actions { display: none !important; }
                    }
                `}</style>
                <Box id="compliance-report-print-actions" sx={{ position: "absolute", right: 12, top: 12, display: "flex", gap: 1 }}>
                    <IconButton
                        onClick={() => window.print()}
                        title="Print or save as PDF"
                        sx={{ bgcolor: "#FFFFFF", border: "1px solid #E5E7EB", "&:hover": { bgcolor: "#E5E7EB" } }}
                    >
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 0.5, pr: 10 }}>
                ...
                {report.status === "pending" && (
                    <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, py: 6 }}>
                        <CircularProgress size={32} sx={{ color: "rgba(139, 92, 246, 1)" }} />
                ...
                {report.status === "complete" && c && (
                    <>
                        ...
                        >
                            ...
                            <Typography sx={{ fontSize: "28px", fontWeight: 600, color: "#111827" }}>
                                {c.overall_score}
                        ...
                        {c.content.ai_analysis && (
                            <Box sx={{ mb: 2.5, p: 2, borderRadius: "18px", bgcolor: "#FAFAFA", border: "1px solid #E5E7EB" }}>
                                <Typography sx={{ fontSize: "11px", fontWeight: 600, color: "#9CA3AF", textTransform: "uppercase", mb: 0.5 }}>
                                    AI Analysis
                                </Typography>
                        ...
                        {report.recommendations.length > 0 && (
                            <Box>
                                <Typography sx={{ fontSize: "11px", fontWeight: 600, color: "#9CA3AF", textTransform: "uppercase", mb: 1 }}>
                                    Recommendations
                                </Typography>
                                <Stack spacing={1}>
                                    {report.recommendations.map((rec, idx) => (
                                        <Box key={idx} sx={{ display: "flex", gap: 1, alignItems: "flex-start" }}>
                                            <Box sx={{ width: 5, height: 5, borderRadius: "50%", bgcolor: "rgba(139, 92, 246, 1)", mt: "7px", flexShrink: 0 }} />
